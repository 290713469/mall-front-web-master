        ...
        <div v-if="loginPage" class="content">
          <ul class="common-form">
            <li class="username border-1p">
              <div class="input">
                <input type="text" v-model="ruleForm.userName" :placeholder="$t('login.account')">
              </div>
            </li>
            <li>
              <div class="input">
                <input type="password" v-model="ruleForm.userPwd" @keyup.enter="login" :placeholder="$t('login.password')">
              </div>
            </li>
            <!--<li>
              <div id="captcha">
            ...
            <li style="text-align: right" class="pr">
              <el-checkbox class="auto-login" v-model="autoLogin">{{$t("login.remember_password")}}</el-checkbox>
              <!-- <span class="pa" style="top: 0;left: 0;color: #d44d44">{{ruleForm.errMsg}}</span> -->
              <a href="javascript:;" class="register" @click="toRegister">{{$t("login.register_account")}}</a>
             <a style="padding: 1px 0 0 10px">{{$t("login.forget_password")}}</a>
            </li>
          </ul>
          <!--登陆-->
          ...
          <!--返回-->
          <div>
            <y-button :text="$t('login.return')" @btnClick="login_back"
...
export default {
  ...
  methods: {
    ...
    messageSuccess () {
      this.$message({
        message: this.$t('login.register_success_msg'),
        type: 'success'
      })
    },
    message (m) {
    ...
    login () {
      this.logintxt = this.$t('login.login_status')
      this.rememberPass()
      if (!this.ruleForm.userName || !this.ruleForm.userPwd) {
        // this.ruleForm.errMsg = '账号或者密码不能为空!'
        this.message(this.$t('login.account_password_empty_msg'))
        return false
      }
      var params = {
      ...
      userLogin(params).then(res => {
        if (res.result.state === 1) {
          ...
          this.logintxt = this.$t('login.login')

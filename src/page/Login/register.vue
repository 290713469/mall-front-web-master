    ...
    <div class="wrapper">
      <div class="dialog dialog-shadow" style="display: block; margin-top: -362px;">
        <div class="registered">
          <h4>{{$t("register.register_msg")}}</h4>
          <div class="content" style="margin-top: 20px;">
            <ul class="common-form">
              <li class="username border-1p">
                <div style="margin-top: 40px;" class="input">
                  <input type="text"
                         v-model="registered.userName" :placeholder="$t('login.account')"
...
export default {
  data () {
    return {
      ...
      agreement: false,
      registxt: this.$t('register.register_mst'),
      statusKey: ''
    }
  },
  computed: {
  ...
  methods: {
    ...
    regist () {
      this.registxt = this.$t('register.register_status')
      ...
      if (!userName || !userPwd || !userPwd2) {
        this.message(this.$t('register.account_password_empty_msg'))
        this.registxt = this.$t('register.register_mst')
        return false
      }
      if (userPwd2 !== userPwd) {
        this.message(this.$t('register.different_password_msg'))
        this.registxt = this.$t('register.register_mst')
        return false
      }
      if (!this.agreement) {
        this.message(this.$t('register.not_agreement_msg'))
        this.registxt = this.$t('register.register_mst')
        return false
      }
      register({
        ...
        statusKey: this.statusKey }).then(res => {
          if (res.success === true) {
            ...
            captcha.reset()
            this.regist = this.$t('register.register_mst')
            return false
          }
        })
    },

<template>
  ...
  <div v-loading="loading" :element-loading-text="$t('goods.loading')" style="min-height: 35vw;" v-if="!error">
   <div class="banner" >
      <div class="bg" ref="bg"
    ...
    <div class="category-panel activity-panel" >
      <div class="box" v-for="(row) in itemCategoryList">
        <div class="category-content" v-for="(iitem,j) in row" :key="j" @click="linkTo(iitem)" >
          <img class="category-img" :src="iitem.icon">
    ...
    <div v-for="(item,i) in home" :key="i">
     ...
     <!--<section class="w mt30 clearfix" v-if="item.type === 2">
      ...
      <section class="w mt30 clearfix" v-if="item.type === 2">
        <y-shelf :title="$t(convertPanelLabel(item.name))" v-if="item.panelContents.length !== 0">
        </y-shelf>
      ...
      <section class="w mt30 clearfix" v-if="item.type === 3">
        ...
        <div class="goods-box w" >
          ...
          <mall-goods :msg="iitem" v-for="(iitem,j) in item.panelContents" :key="j+'key'" ></mall-goods>
...
<script>
  import { productHome, parentCategoryList } from '/api/index.js'
  ...
  export default {
    ...
    methods: {
      ...
      convertPanelLabel (panelName) {
        if (panelName === '热门商品') {
          return 'common.hot_product'
        } else if (panelName === '官方精选') {
          return 'common.vendor_suggestion'
        } else if (panelName === '品牌精选') {
          return 'common.brand_suggestion'
        } else if (panelName === '品牌周边') {
          return 'common.brand_peripheral'
        } else if (panelName === '活动版块') {
    ...
    computed: {
      itemCategoryList: function () {
        ...
        for (var i = 0; i < this.category.length; i++) {
          ...
          arrTemp[index].push(this.category[i])
        }
        return arrTemp
      }
    },

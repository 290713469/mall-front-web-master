<template>
  <div>
    <y-shelf :title="$t('user.account_info')">
      <div slot="content">
        <div class="avatar-box">
          <div class=img-box><img :src="userInfo.info.file" alt=""></div>
          <div class="r-box">
            <h3 style="margin-left: 13px;">{{$t("user.modify_image")}}</h3>
            <y-button :text="$t('user.upload_image')" classStyle="main-btn" style="margin: 0;" @btnClick="editAvatar()"></y-button>
          </div>
        </div>
        <div class="edit-avatar" v-if="editAvatarShow">
          <y-shelf :title="$t('user.set_image')">
        <span slot="right">
            ...
            <div slot="content" class="content">
              <div class="edit-l">
                ...
                <div style="padding: 10px 0 ">{{$t("user.review_image")}}</div>
                <div class="btn">
                  <a href="javascript:;">{{$t("user.modify_image")}}</a>
    ...
    methods: {
      ...
      upimg (e) {
        ...
        if (file.size > 1048576) {
          this.messageFail(this.$t('user.image_size_msg'))
          return false
        }
        if (!/\.(gif|jpg|jpeg|png|bmp|GIF|JPG|PNG)$/.test(e.target.value)) {
          this.messageFail(this.$t('user.image_format_msg'))
          return false
        }
